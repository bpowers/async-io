...
impl Source {
    ...
    fn ready<S: Borrow<crate::Async<T>> + Clone, T>(handle: S, dir: usize) -> Ready<S, T> {
        Ready {
            handle,
            dir,
            ticks: None,
...
struct Ready<S: Borrow<crate::Async<T>>, T> {
    ...
    ticks: Option<(usize, usize)>,
    index: Option<usize>,
    _guard: Option<RemoveOnDrop<S, T>>,
}

impl<S: Borrow<crate::Async<T>>, T> Unpin for Ready<S, T> {}

impl<S: Borrow<crate::Async<T>> + Clone, T> Future for Ready<S, T> {
    type Output = io::Result<()>;
...
struct RemoveOnDrop<S: Borrow<crate::Async<T>>, T> {
    handle: S,
    dir: usize,
    key: usize,
...
impl<S: Borrow<crate::Async<T>>, T> Drop for RemoveOnDrop<S, T> {
    fn drop(&mut self) {
